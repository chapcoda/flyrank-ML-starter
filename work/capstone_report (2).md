...
FlyRank's content-refresh workflow rests on a premise its own March 2026 research makes
explicit: content decays as it ages, so refreshing mature pages is the highest-priority
action a content team can take. Lane 2 of this internship asks whether that prioritisation
can be improved — whether a model can identify which published pages are about to lose
search visibility, early enough for an editor to act. Using 50,840 pages across 37 client
portfolios from the FlyRank internship warehouse — features from February–April 2026,
outcomes from May 2026 — I built a client-held-out Random Forest and compared it against a
baseline encoding exactly that staleness premise. The heuristic did not merely underperform:
it scored a ROC AUC of 0.298, below chance, meaning it ranked pages consistently wrong on
this portfolio, and inverting its output would have scored 0.702 — better than the model.
The model improved on it (Precision@50 of 0.64 against 0.26; AUC 0.638 against 0.298), but
auditing that result surfaced three problems that matter more than the improvement: a
one-directional label contamination in which a client's lapsed analytics connection is
recorded as content decline, affecting 17.7% of positive cases; an evaluation metric that
returned seven different values across seven runs on a fixed seed; and a test set in which
87% of pages belonged to a single client. The output is a per-client review queue —
decision-support for sequencing an editor's week, not a count of declining pages, and not
evidence that refreshing them changes the outcome.

## 1. Problem framing

**The content problem this comes from.** FlyRank manages content portfolios at a scale
where inspection is impossible. The warehouse behind this work covers 37 client portfolios
and roughly 79 million daily page-level records; a single client in it holds over 9,000
published pages. No editor reviews that inventory.

The platform's existing answer comes from FlyRank's own published research, which identifies
content refresh as Priority Action #1: older pages updated recently outperform similarly old
stale pages, so a team should work its oldest high-visibility pages first. That
recommendation is reasonable, widely held across the industry, and is the premise Lane 2 was
set up to test against production data.

**The decision.** The operative question is never "which pages are declining" but "which
twenty should I open this week."
...
Identical model, features, and seed. The naive split inflates Precision@50 by 38% and AUC by
14%. A paper reporting 0.88 would not be lying about the arithmetic — it would be measuring
...
*A third of the apparent hits are unverifiable.* 32 of the top 50 are labelled declining, but
14 of those 32 have no honest May search-console coverage and were labelled declining by the
`COALESCE` mechanism below. Reported Precision@50 is 0.64; the verifiable floor, if every
unverifiable page were wrong, is 0.36.
...
**The primary metric is unstable.** Across seven runs of an identical pipeline — same seed,
same 33,296 rows, same 6 test clients — Precision@50 measured 0.52, 0.58, 0.60, 0.62, 0.64, 0.66, and 0.70. ROC AUC over the same runs stayed within 0.632–0.639. Changing only `n_jobs` from -1 to
...
Inverting the baseline's output gives an AUC of 0.702 — higher than the model's 0.638. The
heuristic contained more usable signal than the model extracted; it was simply pointed the
wrong way.
...
baseline was built on — scores 0.0002 — indistinguishable from zero.
...
All thresholds are relative to the page's own client. Thirty-eight of 52 entries involve weak
click-through; seven involve deep ranking. Two codes never fire — no queue page is stale
...
**Confidence: directional, per client unproven.** Aggregate queue hit rate is 59.6% against a
50.6% base rate. Per client: two are marginally above base rate, two are flat, two fall
below it — no client clearly beats it. An earlier
...
**Check before acting on any entry.** 25 of 52 queue pages have under 100 impressions in
...
*Do not report it as a count of at-risk pages.* Precision@50 of 0.64 has a verifiable floor of
0.36.
...
`work/capstone_metrics.json`, because Precision@50 has varied across library versions.
...
produced are committed at `work/capstone_metrics.json`. That file records both
...
**Artifacts committed.** The queue itself is regenerated by the notebook rather than
committed — the repository's `data-leak-check` workflow blocks committed data files, which
is the control working as intended.

| file | contents |
|---|---|
| `work/capstone_metrics.json` | every headline number, plus model spec and windows |
| | `work/fig1_headline.png` | model vs baseline |
| `work/fig2_inversion.png` | age-bucket concentration and ROC curves |
| `work/fig3_contamination.png` | coverage vs apparent decline, 22 clients |
| `work/fig4_per_client.png` | queue hit rate vs base rate per client |
